## A Teradata Database is Required For These Tests
...
if (teradata_is_working){
  ...
  test_that(
    "get_schemas retrieves schemas correctly",
    {
      ...
      expect_true(
        all(
          c("SysAdmin", "Sys_Calendar", "DBC") %in% get_schemas_teradata(con)
        )
      )
  ...
  test_that(
    "get_tables retrieves tables correctly",
    {
      ...
      expect_true(
        "mtcars" %in% get_tables_teradata(
          con,
          schema = "mc678p"
        )
      )

      # res <- DBI::dbSendQuery(con, "DROP TABLE mc678p.mtcars")
      # DBI::dbClearResult(res)
